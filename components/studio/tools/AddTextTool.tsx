...
export default function AddTextTool({ baseFile, onTextApplied }: AddTextToolProps) {
        ...
        const loadPdf = async () => {
            try {
                ...
                if (!cancelled) {
                    ...
                    notify("Could not load PDF document.");
                }
            } finally {
    ...
    const handleTextProcessing = () => {
        ...
        if (validElements.length === 0) {
            notify("Please add at least one text box with text content.");
            return;
        }
        ...
        requireAuth(async () => {
            try {
                ...
                setSuccess(true);
                notify("Text added PDF loaded back into Studio.");
            } catch (err) {
                console.error(err);
                setError(getFriendlyErrorMessage(err));
                    ...
                    <div className="rounded-2xl border border-[color:var(--border)] bg-[var(--card)] p-6 space-y-5">
                        ...
                        <div className="space-y-3 rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)]/40 p-4">
                            ...
                            <div className="flex items-center justify-between text-xs text-[color:var(--muted)]">
                                ...
                                <span className="font-semibold text-[color:var(--foreground)]">{elements.length}</span>
                            </div>
                        </div>
                        ...
                        {activeElement ? (
                            <div className="space-y-4 animate-fadeIn">
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        className="min-h-[110px] w-full rounded-xl border border-[color:var(--border)] bg-[color:var(--background)] px-4 py-3 text-sm font-medium text-[color:var(--foreground)] outline-none transition focus:border-indigo-500"
                                    />
                                </div>
                            ...
                            <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-[color:var(--border)] py-6 text-center text-[color:var(--muted)]">
            ...
            <div className="flex min-h-0 flex-col lg:col-span-7">
                <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)]/30 p-4">
                    ...
                    >
                        ...
                        <div
                            ref={containerRef}
                            className="relative inline-block rounded border border-gray-400/20 bg-white shadow-xl"
                                ...
                                return (
                                    <div
                                        ...
                                        className={`cursor-move transition-shadow ${
                                            isActive
                                                ? "z-20 rounded bg-indigo-500/5 shadow-lg ring-2 ring-indigo-500"
                                                : "z-10 hover:ring-1 hover:ring-[color:var(--border)]"
                                        }`}
                                    >
                                        <textarea
                                            value={el.text}
                                            ...
                                            className="m-0 min-h-[1.5em] min-w-[50px] resize-none overflow-hidden rounded-md border-none bg-transparent p-1 font-semibold outline-none"
                                            onInput={(e) => {
