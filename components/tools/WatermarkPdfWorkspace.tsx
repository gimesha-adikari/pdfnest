                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full">
                    <div className="lg:col-span-5 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2 bg-[color:var(--background)]/50 p-1 rounded-xl border border-[color:var(--border)]">
                                <button
                                    type="button"
                        ...
                        {watermarkType === "image" && (
                            <div className="space-y-2 animate-in fade-in-50 duration-150">
                                ...
                                {!imageFile ? (
                                    <div className="relative border border-dashed border-[color:var(--border)] hover:border-indigo-500 rounded-xl p-6 bg-[color:var(--background)]/30 transition flex flex-col items-center justify-center text-center cursor-pointer">
                                        <input type="file" accept="image/png, image/jpeg, image/jpg" onChange={handleImageChange} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                                        <ImageIcon size={24} className="text-[color:var(--muted)] mb-2" />
                                        <p className="text-xs font-bold text-[color:var(--foreground)]">Select Watermark Image</p>
                                        <p className="text-[10px] text-[color:var(--muted)] mt-0.5">Supports transparent PNG, JPG signatures</p>
                                    </div>
                                ) : (
                                    <div className="flex items-center justify-between border border-[color:var(--border)] p-3 rounded-xl bg-[color:var(--background)]/50">
                                        <div className="flex items-center gap-3 min-w-0">
                                            <img src={imagePreview} className="w-10 h-10 object-contain rounded bg-[color:var(--border)]/20 p-0.5 border" alt="preview" />
                                        ...
                                        <button type="button" onClick={() => { setImageFile(null); setImagePreview(""); }} className="p-1.5 hover:bg-red-500/10 rounded-lg text-red-500 transition shrink-0">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                )}
                        ...
                        <div className="space-y-4 pt-2 border-t border-[color:var(--border)]">
                            <div className="space-y-2">
                                <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-wider text-[color:var(--muted)]">
                                    <span>Sizing Scale Dimensions</span>
                                    <span className="font-mono text-indigo-500">{fontSize}%</span>
                                </div>
                                <input type="range" min="10" max="300" value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} className="w-full accent-indigo-500" />
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <input type="range" min="-180" max="180" value={rotation} onChange={(e) => setRotation(Number(e.target.value))} className="w-full accent-indigo-500" />
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <input type="range" min="0.05" max="1.0" step="0.05" value={opacity} onChange={(e) => setOpacity(Number(e.target.value))} className="w-full accent-indigo-500" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2 pt-2 border-t border-[color:var(--border)]">
                            ...
                            <div className="grid grid-cols-3 gap-1.5 bg-[color:var(--background)]/50 p-1.5 border border-[color:var(--border)] rounded-xl max-w-[240px]">
                    ...
                    <div className="lg:col-span-7 flex flex-col justify-between bg-[color:var(--background)]/30 border border-[color:var(--border)] rounded-2xl p-6 relative min-h-[500px]">
                        {isRenderingCanvas && (
                        ...
                        <div className="w-full flex items-center justify-between border-b border-[color:var(--border)] pb-3 text-[color:var(--foreground)] text-sm font-bold mb-4">
                            ...
                            {totalPages > 0 && (
                                <div className="flex items-center gap-2 border border-[color:var(--border)] px-2 py-0.5 rounded-lg bg-[var(--card)] text-xs text-[color:var(--muted)] font-mono">
                                    <button type="button" disabled={currentPage <= 1} onClick={() => setCurrentPage(prev => prev - 1)} className="hover:text-[color:var(--foreground)] disabled:opacity-20">
                                        <ChevronLeft size={14} />
                                    </button>
                                    <span>Page {currentPage} of {totalPages}</span>
                                    <button type="button" disabled={currentPage >= totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="hover:text-[color:var(--foreground)] disabled:opacity-20">
                        ...
                        <div className="flex-1 w-full flex items-center justify-center bg-gray-500/5 dark:bg-black/20 rounded-xl border border-[color:var(--border)] relative overflow-hidden p-4 select-none min-h-[380px]">
                            <div className="relative shadow-xl rounded border border-gray-400/20 bg-white max-w-full">
                                ...
                                >
                                    {watermarkType === "text" ? (
                                        <span className={`whitespace-nowrap font-black text-red-500 select-none ${fontStyleClass}`}>
                                            {watermarkText || "STAMP"}
                                        </span>
                                    ) : (
                                        imagePreview ? (
                                            <img src={imagePreview} className="w-full h-auto object-contain select-none pointer-events-none" alt="watermark graphics preview layer widget" />
                                        ) : (
                                            <span className="text-[10px] uppercase font-mono font-bold tracking-widest text-indigo-500 bg-indigo-500/10 px-2 py-1 rounded border border-indigo-500/20 select-none">IMAGE NOT ATTACHED</span>
                                        )
                                    )}
                                </div>
                            </div>
                        ...
                        {success && (
                            <div className="w-full mt-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-emerald-900 dark:text-emerald-200 flex items-start gap-3">
                                ...
                                <div className="text-xs">
                                    ...
                                    <p className="mt-0.5 text-emerald-800/80 dark:text-emerald-200/70">The system has successfully burned your custom layout configurations into a new asset download file.</p>
                                </div>
                            </div>
                        )}
