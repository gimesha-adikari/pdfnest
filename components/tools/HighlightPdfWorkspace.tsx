...
import { Highlighter, Loader2, ShieldCheck, ChevronLeft, ChevronRight, Eye, Trash2, Undo2, Redo2 } from "lucide-react";
...
interface PdfJsDocument { numPages: number; getPage: (pageNumber: number) => Promise<PdfJsPage>; }
...
export default function HighlightPdfWorkspace() {
    ...
    // UNDO / REDO STATE HISTORICAL MANIFEST TRACKERS
    ...
    // Action execution wrapper to snapshot changes into history
    const updateBoxesStateWithHistory = (nextState: HighlightBox[] | ((prev: HighlightBox[]) => HighlightBox[])) => {
        setBoxes(prev => {
            const computedNext = typeof nextState === "function" ? nextState(prev) : nextState;
            setHistoryPast(past => [...past, prev]);
            setHistoryFuture([]); // Clearing redo paths upon a fresh mutation timeline split
            return computedNext;
        });
    };
    ...
    // Global Key Event Listeners for Keyboard Shortcuts Capture Structure
    useEffect(() => {
        const handleKeyboardShortcuts = (e: KeyboardEvent) => {
            ...
            if (isMeta && keyLower === "z") {
                e.preventDefault();
                if (e.shiftKey) {
                    handleRedoAction(); // Shift + Cmd/Ctrl + Z layout logic tracking
                } else {
                    handleUndoAction();
                }
    ...
    const handleContainerPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        // Snapshot history timeline safely
    ...
    const handleContainerPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        // Update working geometry slice directly without filling past stacks per coordinate point move
    ...
    return (
        <>
            <PdfToolHero
                ...
                description="Click and drag over text blocks on the document viewport to add custom translucent highlighting markers cleanly."
            />
            ...
            <div className="mt-12 rounded-3xl border border-[color:var(--border)] bg-[var(--card)] p-8 shadow-lg w-full">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <PdfFileInfo file={activeFile} onClear={() => {
                            setFile(null);
                            router.push(`/${toolId}`);
                        }} />
                        ...
                        <div className="rounded-2xl border border-[color:var(--border)] p-5 bg-[color:var(--background)]/50 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                {/* INTERACTIVE UNDO AND REDO BAR UTILITY CONTROLS */}
                                ...
                                <div className="flex items-center gap-2">
                                    {HIGHLIGHT_COLORS.map(c => (
                                        <button
                                            ...
                                            className={`h-8 w-8 rounded-full border-2 transition ${selectedColor === c.hex ? 'border-indigo-600 scale-110 shadow-md' : 'border-transparent hover:scale-105'}`}
                                            title={c.name}
                                        />
                                    ))}
                                </div>
                    ...
                    {/* Interactive Canvas Workspace Viewport */}
                    <div className="lg:col-span-7 flex flex-col bg-[color:var(--background)]/30 border border-[color:var(--border)] rounded-2xl p-6 relative w-full">
                        ...
                        <div className="w-full flex items-center justify-between border-b border-[color:var(--border)] pb-3 text-[color:var(--foreground)] text-sm font-bold mb-4">
                            <span className="flex items-center gap-2"><Eye size={16} className="text-indigo-500" /> Highlighter Canvas</span>
                            {totalPages > 0 && (
                                <div className="flex items-center gap-2 border border-[color:var(--border)] px-2 py-0.5 rounded-lg bg-[var(--card)] text-xs text-[color:var(--muted)] font-mono select-none">
                                    <button type="button" disabled={currentPage <= 1} onClick={() => { setCurrentPage(p => p - 1); setActiveId(null); }} className="hover:text-[color:var(--foreground)] disabled:opacity-20 transition"><ChevronLeft size={14} /></button>
                                    <span>Page {currentPage} of {totalPages}</span>
                                    <button type="button" disabled={currentPage >= totalPages} onClick={() => { setCurrentPage(p => p + 1); setActiveId(null); }} className="hover:text-[color:var(--foreground)] disabled:opacity-20 transition"><ChevronRight size={14} /></button>
                                </div>
                            )}
                        </div>
                                ...
                                {boxes.filter(box => box.page === currentPage).map(box => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`opacity-40 transition-all ${isActive ? 'ring-2 ring-indigo-600 opacity-60 z-20 shadow-md' : 'hover:opacity-50 z-10'}`}
                                        />
                                    );
                                })}
                            </div>
