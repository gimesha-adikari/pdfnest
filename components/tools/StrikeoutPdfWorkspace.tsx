...
export default function StrikeoutPdfWorkspace() {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            try {
                ...
                setDownloadData({
                    blob,
                    fileName: `${fileName}-strikeout.pdf`,
                });
                ...
                notify("Strikeout PDF ready.", "success");
    ...
    const handleStrikeoutProcessing = () => {
        ...
        requireAuth(async () => {
            try {
                ...
                if (strikeoutMode === "ocr" || (isScannedPage && strikeoutMode === "smart")) {
                    ...
                    notify("Strikeout job queued. Waiting for OCR worker...", "info");
                    return;
                }
                ...
                if (result.executionMode === "client") {
                    const fileName = (file?.name || "document.pdf").replace(/\.pdf$/i, "");
                    const struckoutFile = new File([result.blob], `${fileName}-struckout.pdf`, {
                        type: "application/pdf",
                    });

                    setDownloadData({
                        blob: result.blob,
                        fileName: `${fileName}-struckout.pdf`,
                    });
    ...
    return (
        <>
            <PdfToolHero
                title="Strikeout PDF Text"
                description="Select text-like regions, choose Smart / Manual / OCR, and permanently strike through the selected text."
            />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 w-full">
                    <div className="lg:col-span-5 space-y-6">
                        ...
                        <div className="rounded-2xl border border-[color:var(--border)] p-5 bg-[color:var(--background)]/50 space-y-4">
                                ...
                                <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/10 p-4 flex items-start gap-3">
                                    ...
                                    <div className="text-xs text-[color:var(--foreground)]/90">
                                        ...
                                        <p className="mt-0.5 text-[color:var(--muted)]">
                                            Detecting text pages before strikeout processing.
                                        </p>
                                    </div>
                                </div>
                            )}
                            ...
                            <PdfActionButton
                                text="Save Strikeouts"
                                loadingText="Applying Strikeouts..."
                        ...
                        >
                            ...
                            >
                                ...
                                {boxes.filter((box) => box.page === currentPage).map((box) => {
                                    ...
                                    const isOcrBox = strikeoutMode === "ocr";
                                    return (
                                        <div
                                            key={box.id}
                                            onClick={(e) => {
