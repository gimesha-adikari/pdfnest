...
import { Image as ImageIcon, ShieldCheck, LayoutGrid, Layers, Move, SlidersHorizontal, Undo2, Redo2, PlusCircle, Trash2 } from "lucide-react";
...
type InteractionMode = "move" | "resize-nw" | "resize-n" | "resize-ne" | "resize-e" | "resize-se" | "resize-s" | "resize-sw" | "resize-w";
                ...
                <div className="flex border-b border-[color:var(--border)] mb-8 gap-2 items-center justify-between">
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            {!isPro && (
                                <span className="ml-1.5 text-[9px] font-black bg-amber-500/20 text-amber-500 border border-amber-500/30 px-1.5 py-0.5 rounded-md uppercase tracking-wider">
                                    Pro
                                </span>
                            )}
                        </button>
                    ...
                    <div className="mt-8 space-y-6">
                        ...
                        {activeTab === "standard" && (
                            <>
                                <div className="w-full flex items-center justify-between border-b border-[color:var(--border)] pb-2">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => { setImages([]); setCanvasItems([]); setFile(null); router.push(`/${toolId}`); }}
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                    {images.map((img, index) => {
                                        ...
                                        return (
                                            <div key={`${img.name}-${index}`} className="group relative border border-[color:var(--border)] rounded-xl overflow-hidden aspect-square bg-[color:var(--background)] shadow-sm">
                                                <img src={localPreviewUrl} alt={img.name} className="w-full h-full object-cover" onLoad={() => URL.revokeObjectURL(localPreviewUrl)} />
                                                <button type="button" onClick={() => removeImageElement(index)} className="absolute top-1.5 right-1.5 bg-red-500 text-white rounded-full p-1 text-xs opacity-0 group-hover:opacity-100 transition shadow-md">✕</button>
                                                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-2 text-[10px] text-white truncate">{img.name}</div>
                                            </div>
                                        );
                                    })}
                                </div>
                        ...
                        {activeTab === "custom" && isPro && (
                            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-[600px]">
                                            ...
                                            <div key={`page-${pageIdx}`} className="flex flex-col items-center gap-2 shrink-0">
                                                <div className="w-full flex items-center justify-between text-xs font-bold text-[color:var(--muted)] px-1">
                                                    ...
                                                    {itemsOnThisPage.length === 0 && (
                                                        <span className="text-[10px] text-amber-500 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded font-medium">Empty (Will be skipped on export)</span>
                                                    )}
                                                </div>
                                                <div
                                                ...
                                                >
                                                    <div className="absolute inset-0 pointer-events-none opacity-[0.03] bg-[linear-gradient(to_right,#000_1px,transparent_1px),linear-gradient(to_bottom,#000_1px,transparent_1px)] bg-[size:20px_20px]" />
                                                    ...
                                                    {itemsOnThisPage.map((item) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <img src={imgUrl} alt={item.name} className="w-full h-full object-cover pointer-events-none" onLoad={() => URL.revokeObjectURL(imgUrl)} />
                                                                ...
                                                                >
                                                                    <Move className="text-white opacity-0 group-hover:opacity-40 drop-shadow-md transition" size={20} />
                                                                </div>

                                                                {isSelected && (
                                                                    <>
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-nw")} className="absolute -top-1.5 -left-1.5 w-3 h-3 bg-indigo-600 border border-white rounded-full cursor-nwse-resize z-50 shadow" />
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-ne")} className="absolute -top-1.5 -right-1.5 w-3 h-3 bg-indigo-600 border border-white rounded-full cursor-nesw-resize z-50 shadow" />
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-se")} className="absolute -bottom-1.5 -right-1.5 w-3 h-3 bg-indigo-600 border border-white rounded-full cursor-nwse-resize z-50 shadow" />
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-sw")} className="absolute -bottom-1.5 -left-1.5 w-3 h-3 bg-indigo-600 border border-white rounded-full cursor-nesw-resize z-50 shadow" />

                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-n")} className="absolute -top-1 inset-x-4 h-1 bg-indigo-500/40 hover:bg-indigo-600 cursor-ns-resize z-40 transition" />
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-s")} className="absolute -bottom-1 inset-x-4 h-1 bg-indigo-500/40 hover:bg-indigo-600 cursor-ns-resize z-40 transition" />
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-e")} className="absolute -right-1 inset-y-4 w-1 bg-indigo-500/40 hover:bg-indigo-600 cursor-ew-resize z-40 transition" />
                                                                        <div onPointerDown={(e) => handleInteractionStart(e, item, "resize-w")} className="absolute -left-1 inset-y-4 w-1 bg-indigo-500/40 hover:bg-indigo-600 cursor-ew-resize z-40 transition" />
                                                                    </>
                                                                )}

                                                                <div className="absolute bottom-0 inset-x-0 bg-black/60 text-[8px] text-white p-0.5 opacity-0 group-hover:opacity-100 truncate pointer-events-none">
                                ...
                                <div className="border border-[color:var(--border)] rounded-2xl p-5 bg-[var(--card)]/50 space-y-6">
                                    ...
                                    {leadingItem ? (
                                        <div className="space-y-4 text-xs">
                                            <div className="flex items-center justify-between gap-2 border-b border-[color:var(--border)] pb-2">
                                            ...
                                            <div className="space-y-1">
                                                <label className="block text-[10px] text-[color:var(--muted-foreground)] font-semibold mb-1">Assigned Page Workspace</label>
                                                <select
                                                ...
                                                >
                                                    {Array.from({ length: customPagesCount }).map((_, idx) => (
                                                        <option key={`opt-${idx}`} value={idx}>Page Layout Sheet {idx + 1}</option>
                                                    ))}
                                                </select>
                                            </div>

                                            <div className="space-y-1">
                                                <div className="flex justify-between text-[10px] text-[color:var(--muted-foreground)] font-semibold">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        updateSelectedItemsFields(item => {
                                                            ...
                                                            return { width: nextW, height: Math.round(nextW / item.aspectRatio) };
                                                        });
                                                    }}
                                                    onMouseUp={() => pushToHistory(canvasItems)}
                                                    className="w-full accent-indigo-500 h-1 bg-[color:var(--border)] rounded-lg appearance-none cursor-pointer"
                                            ...
                                            <div className="pt-2">
                                                <label className="block text-[10px] text-[color:var(--muted)] mb-1 font-bold">Border Hue</label>
                                                <input
                                                    type="color"
                                        ...
                                        <p className="text-[11px] text-[color:var(--muted)] italic text-center py-8">Select one or more image nodes on the canvas to display real-time touch adjustments.</p>
                                    )}
                                </div>
                            </div>
                        )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="relative w-full max-w-md bg-[color:var(--card)] border border-[color:var(--border)] rounded-3xl p-8 shadow-2xl text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
                        ...
                        <p className="mt-3 text-xs leading-relaxed text-[color:var(--muted)] font-medium px-2">
                            Multi-page vector scaling transformations, layer orders (Z-index properties), border customizations, and matrix positioning configurations are restricted to Pro tier accounts.
                        </p>
