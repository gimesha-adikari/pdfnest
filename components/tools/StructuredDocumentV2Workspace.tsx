...
import {
    ...
    StructuredOcrV2ApiError, projectStructuredWarnings,
...
export default function StructuredDocumentV2Workspace({ profile }: Props) {
    const { openAuthModal, requireAuth, isAuthenticated, isLoggedIn, isLoading: authLoading } = useAuth();
    const { file, setFile } = useSharedTool();
    const isMarkdown = profile === "PDF_MARKDOWN_V2";
    const canUseProduct = isMarkdown ? isLoggedIn : isAuthenticated;
    ...
    const canSubmit = Boolean(file && canUseProduct && capabilities && languageReady && (effectiveState === "FILE_READY" || languageFallback));
    ...
    const formattingNotes = useMemo(() => projectStructuredWarnings(result?.warnings || []), [result?.warnings]);
    ...
    useEffect(() => {
        if (!jobId || !active || !canUseProduct) return;
        let mounted = true;
    ...
    }, [active, canUseProduct, jobId, profile]);
    ...
    return <>
        ...
        <div className="mx-auto mt-10 max-w-5xl space-y-6">
            <section className="rounded-3xl border border-[color:var(--border)] bg-[var(--card)] p-5 shadow-lg sm:p-8">
                ...
                {file && !active && !result && (effectiveState === "FILE_READY" || languageFallback) && <div className="mt-6 flex flex-col gap-3"><button type="button" onClick={() => requireAuth(() => { if (!authLoading && canUseProduct) void submit(); })} disabled={!canSubmit || authLoading} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><ShieldCheck size={16} /> {isMarkdown ? "Convert to Markdown" : "Extract data"}</button>{!canSubmit && <p className="text-center text-xs font-semibold text-amber-600" role="status">{!canUseProduct ? "Sign in required" : !capabilities ? "Languages unavailable" : !languageReady ? "Choose at least one language" : "Choose a PDF first"}</p>}</div>}
            ...
            {result && state === "SUCCEEDED" && <section className="overflow-hidden rounded-3xl border border-[color:var(--border)] bg-[var(--card)] shadow-lg"><div className="flex flex-col gap-3 border-b border-[color:var(--border)] p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><CheckCircle2 className="text-emerald-500" size={22} /><div><h2 className="text-lg font-bold">{isMarkdown ? "Your Markdown result" : "Your structured document result"}</h2><p className="mt-1 text-xs text-[color:var(--muted)]">{result.pages.length} page{result.pages.length === 1 ? "" : "s"} · Formatting preserved where supported</p></div></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void copy()} className="inline-flex items-center gap-2 rounded-xl border border-[color:var(--border)] px-3 py-2 text-xs font-semibold"><Clipboard size={14} /> {copyState ? "Copied" : "Copy"}</button><button type="button" onClick={download} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-semibold text-white"><Download size={14} /> Download {isMarkdown ? ".md" : ".json"}</button><button type="button" onClick={reset} className="inline-flex items-center gap-2 rounded-xl border border-[color:var(--border)] px-3 py-2 text-xs font-semibold"><RotateCcw size={14} /> New document</button></div></div>{formattingNotes.length > 0 && <details className="border-b border-amber-500/20 bg-amber-500/5 px-5 py-4 text-xs text-amber-800 dark:text-amber-200"><summary className="cursor-pointer font-semibold">Formatting notes</summary><ul className="mt-2 list-disc space-y-1 pl-5">{formattingNotes.map((note) => <li key={note}>{note}</li>)}</ul></details>}<pre className="max-h-[620px] overflow-auto whitespace-pre-wrap break-words bg-[var(--background)] p-5 text-sm leading-7">{resultText}</pre></section>}
            {!canUseProduct && !authLoading && <p className="text-center text-xs text-[color:var(--muted)]">Sign in is required before the durable job can be submitted.</p>}
        </div>
    </>;
}
