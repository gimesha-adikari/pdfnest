    ...
    return <>
        ...
        <div className="mx-auto mt-10 max-w-5xl space-y-6">
            <section className="rounded-3xl border border-[color:var(--border)] bg-[var(--card)] p-5 shadow-lg sm:p-8">
                ...
                {file && !active && !result && <div className="mt-7 grid gap-5 sm:grid-cols-2"><label className="rounded-2xl border border-[color:var(--border)] bg-[var(--background)]/40 p-5"><span className="text-sm font-bold">Language</span><span className="mt-1 block text-xs text-[color:var(--muted)]">Select the installed OCR language for scanned pages.</span><select aria-label="Structured OCR language" value={language} onChange={event => setLanguage(event.target.value)} className="mt-5 w-full rounded-xl border border-[color:var(--border)] bg-[var(--card)] px-3 py-3 text-sm"><option value="eng">English (eng)</option><option value="sin">Sinhala (sin)</option><option value="tam">Tamil (tam)</option></select></label><div className="rounded-2xl border border-[color:var(--border)] bg-[var(--background)]/40 p-5"><div className="flex items-center gap-2 text-sm font-bold"><ShieldCheck size={17} className="text-emerald-500" /> Processing policy</div><p className="mt-1 text-xs text-[color:var(--muted)]">The server keeps native-first routing and chooses the available local engine.</p><select aria-label="Structured routing policy" value={routingPolicy} onChange={event => setRoutingPolicy(event.target.value as StructuredOcrV2RoutingPolicy)} className="mt-5 w-full rounded-xl border border-[color:var(--border)] bg-[var(--card)] px-3 py-3 text-sm"><option value="AUTO">Automatic</option><option value="FAST">Fast local path</option><option value="QUALITY">Quality path</option></select></div></div>}
