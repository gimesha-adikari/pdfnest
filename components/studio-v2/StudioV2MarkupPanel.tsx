...
import { Loader2, Redo2, Trash2, Undo2, X, ScanText, Sparkles, Wand2, PencilLine, Palette, SquareStack, ShieldCheck } from "lucide-react";
import {
  StudioJobDTO,
  StudioMarkupAction,
  StudioMarkupBox,
...
export const StudioV2MarkupPanel: React.FC<StudioV2MarkupPanelProps> = ({
  ...
  return (
    <section className="studio-v2-markup-panel space-y-3" aria-label="Markup tools">
      <div className="studio-v2-markup-section-heading"><span><Wand2 size={14} />Markup tool</span><em>{boxes.length ? `${boxes.length} pending` : "Ready"}</em></div>
      <div className="studio-v2-markup-intro">
      ...
      <div className="studio-v2-markup-section-heading"><span><Wand2 size={14} />Detection mode</span></div>
      <div className="studio-v2-markup-card studio-v2-markup-mode-card flex items-center justify-between gap-2">
        <div className="grid flex-1 grid-cols-3 gap-1" role="radiogroup" aria-label="Markup mode">
      ...
      <div className="studio-v2-markup-section-heading"><span><PencilLine size={14} />Markup action</span></div>
      <div className="studio-v2-markup-card grid grid-cols-3 gap-1" role="group" aria-label="Markup action">
      ...
      <div className="studio-v2-markup-section-heading"><span><Palette size={14} />Appearance</span></div>
      <div className="studio-v2-markup-card studio-v2-markup-appearance"><StudioV2ColorPicker value={color} onChange={onColorChange} label={`${actionLabels[action]} color`} disabled={busy || disabled} testId="studio-markup-color" /></div>

      <div className="studio-v2-markup-card rounded border border-[#292D35] bg-[#14171C] p-2 text-[10px] text-[#D8DCE3]" data-testid="studio-markup-guidance" aria-live="polite">
      ...
      <div className="studio-v2-markup-section-heading"><span><SquareStack size={14} />Pending regions</span><em>{boxes.length ? `${boxes.length} total` : "None"}</em></div>
      <div className="studio-v2-markup-card rounded border border-[#292D35] bg-[#14171C] p-2 text-[10px] text-[#D8DCE3]" aria-live="polite">
      ...
      <div className="studio-v2-markup-card studio-v2-markup-guidance rounded border border-[#292D35] bg-[#14171C] p-2 text-[10px] text-[#D8DCE3]"><div className="flex items-start gap-2"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#b3a0df]" /><div><strong className="block text-xs text-[#ece8f5]">Pending markup stays editable</strong><p className="mt-1 text-[10px] leading-4 text-[#9AA1AD]">Apply creates a new production version. Cancel asks before discarding a draft.</p></div></div></div>
