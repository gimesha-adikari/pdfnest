...
export const StudioV2EditWorkspace: React.FC<Props> = ({ sessionId, baseVersionId, documentName, newIdempotencyKey, onBack, onCompiled }) => {
  ...
  const submitExtract = useCallback(async () => {
    ...
    try {
      ...
      // Set the controlled error here rather than only in the effect caller.
      // React development effect cleanup can otherwise suppress the first
      // rejected submission and leave a no-job editor in a waiting state.
  ...
  }, [busy, job, sessionId, submitExtract]);

  const page = layout?.pages[pageIndex] ?? null;
  const pages = layout?.pages ?? [];
  const extractionView = editorExtractionView({ busy, hasJob: Boolean(job), hasState: Boolean(state), error });
  ...
  const compile = async () => {
    ...
    } catch { setError(editorCompileError); }
    finally {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      else if (response.job.status === "failed" || response.job.status === "cancelled") setError(editorCompileError);
      else window.setTimeout(() => void poll(), 800);
    };
    void poll();
    return () => { cancelled = true; };
  ...
  useEffect(() => {
    ...
    void studioV2Api.getJob(sessionId, saved).then(async (response) => {
      ...
      if (!cancelled) setError(editorCompileError);
    });
    return () => { cancelled = true; };
  ...
  if (extractionView === "extracting") return <div className="flex h-screen items-center justify-center bg-[#0B0C0F] text-white"><Loader2 className="mr-2 animate-spin" /> Extracting editor layout…</div>;
  return <main className="flex h-screen w-screen flex-col overflow-hidden bg-[#0B0C0F] text-[#F5F7FA]" data-testid="studio-edit-workspace">
    ...
    {!state ? <section className="flex flex-1 flex-col items-center justify-center gap-3">{extractionView === "failure" ? <><p className="text-sm text-red-200">{editorExtractionStartError}</p><button type="button" data-testid="studio-editor-extraction-retry" onClick={retryExtract} disabled={busy || Boolean(job)} className="rounded border border-red-300/50 px-3 py-2 text-xs text-red-100 disabled:opacity-50">Retry extraction</button></> : <><Loader2 className="animate-spin text-violet-300" /><p className="text-sm">Waiting for the editor layout…</p></>}</section> : <section className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-hidden p-4 lg:grid-cols-[220px_minmax(0,1fr)_300px]">
