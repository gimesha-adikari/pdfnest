...
test("guest PDF-to-Markdown remains explicitly authenticated-only", async ({ page }) => {
    ...
    await expect(page.getByText("Sign in to convert this document.", { exact: true })).toBeVisible();
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, "guest-policy.json"), `${JSON.stringify({
        ...
        workspace_auth_state: "SIGN_IN_REQUIRED",
