...
function fixtureSha256(): string {
    return execFileSync("sha256sum", [FIXTURE], { encoding: "utf8" }).trim().split(/\s+/)[0];
}

function inspectMarkupOutput(outputPath: string, action: Action, expectedPhrase: string): Record<string, unknown> {
    const script = [
...
async function selectScannedHeading(page: import("@playwright/test").Page): Promise<string> {
    ...
    const texts = await words.evaluateAll((items) => items.map((item) => item.getAttribute("aria-label")?.replace(/^Select\s+/, "") || ""));
    ...
    test("visually selects and durably marks a real scanned fixture for all operations", async ({ page }) => {
        ...
        for (const action of ACTIONS) {
            ...
            try {
                ...
                await actionPage.getByTestId("markup-v2-file-input").setInputFiles(FIXTURE);
                ...
                results.push({ action, job_id: created.job_id, source_sha256: hash, page_count: 3, selected_phrase: phrase, result_preview: { backend_rendered: true, client_canvas_count: 0 }, output: validation });
            } finally {
