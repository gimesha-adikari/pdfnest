...
test.describe("Wave 9B: Markdown to PDF Real Browser Acceptance & Offline Isolation Suite", () => {
    test("1. Device Mode: Converts Markdown file locally with 0 backend network upload and renders preview", async ({
        page,
    }) => {
        let backendConvertCallCount = 0;
        let cdnCallCount = 0;

        await page.route("**/api/conversion/markdown-to-pdf**", async (route) => {
        ...
        // Staging sample Markdown note
        const sampleMd = `
# Engineering Architecture Report

This document outlines the **Client-First Hybrid Architecture** of Platen.

## Core Capabilities

- **100% Offline Device Execution**
- Pure Vector PDF output via \`pdf-lib\`
...
> All document manipulations execute in the client sandbox with zero telemetry.
`;
        ...
        await fileInput.setInputFiles({
            ...
            buffer: Buffer.from(sampleMd),
        });

        // Verify Workspace Elements
        await expect(page.locator("text=Page Layout Metrics")).toBeVisible({ timeout: 10000 });
        ...
        const previewImg = page.locator("img[alt='Markdown Layout Document Snapshot Mirror']");
        await expect(previewImg).toBeVisible({ timeout: 15000 });

        // Click Save Button
        const saveButton = page.getByRole("button", { name: "Save Markdown Vector PDF" });
        await expect(saveButton).toBeEnabled();
        await saveButton.click();

        // Must redirect to download screen
        await expect(page).toHaveURL(/\/markdown-to-pdf\/download/, { timeout: 15000 });
    ...
    test("2. Zero Network Leakage during full Device mode execution", async ({ page }) => {
...
Testing zero network leakage for client-side markdown compiling.
...
`;
    ...
    test("3. Auto Mode Fallback: Non-Latin script with Cloud HTTP 429 quota response displays friendly business error", async ({
        ...
        // Non-Latin source markdown triggers UNSUPPORTED_CLIENT_OP -> Auto Mode Cloud Fallback
        const sinhalaMd = `
# සිංහල ලේඛනය

මෙය ප්ලැටන් මඟින් සකසන ලද සිංහල සටහනකි.
`;
        ...
        await fileInput.setInputFiles({
            name: "sinhala.md",
            mimeType: "text/markdown",
            buffer: Buffer.from(sinhalaMd),
        });

        await expect(page.locator("text=Page Layout Metrics")).toBeVisible({ timeout: 10000 });

        // Verify the toast displays the backend's friendly error message rather than generic CLOUD_UNAVAILABLE
        await expect(page.locator("text=You've reached your 3-hour usage limit.").first()).toBeVisible({ timeout: 15000 });
