    ...
    return (
        <div className="min-h-screen bg-[var(--background)] p-8 text-[color:var(--foreground)] relative">
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                {/* Header */}
                <div
                    className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-[var(--card)] p-6 border border-[color:var(--border)] rounded-2xl">
                    <div>
                        <h1 className="text-3xl font-black flex items-center gap-3">
                            <ShieldAlert className="text-indigo-500"/> Advanced Admin Control Panel
                        </h1>
                        <p className="text-[color:var(--muted-foreground)] text-xs mt-2">Modify live user states, manage
                            limits, inspect user transaction data ledgers, and check platform health logs.</p>
                    </div>
                    <Link
                        href="/admin/content"
                ...
                {activeTab === "users" && (
                    <div
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            {usersList.map((u) => (
                                <tr key={u.ID}
                                    className="border-b border-[color:var(--border)] last:border-0 hover:bg-[color:var(--background)]/30 transition-colors">
                                    <td className="p-4 font-medium flex items-center gap-2">
                                        {u.Email}
                                        {u.Email === MASTER_EMAIL && (
                                            <span title="Immune Master Account"><ShieldCheck size={16}
                                                                                             className="text-indigo-500"/></span>
                                        )}
                                    </td>
                                    <td className="p-4">
                                            <span
                                                className={`px-2 py-1 rounded-md text-xs font-black uppercase ${u.Role === 'admin' ? 'bg-indigo-500/10 text-indigo-500' : 'bg-neutral-500/10 text-neutral-400'}`}>
                                                {u.Role}
                                            </span>
                                    </td>
                                    <td className="p-4">
                                            <span
                                                className={`px-2 py-1 rounded-md text-xs font-bold uppercase ${u.Status === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                                                {u.Status}
                                            </span>
                                    </td>
                                    ...
                                    <tr key={sub.ID}
                                        ...
                                        <td className="p-4">
                                            {isEditing ? (
                                                ...
                                                <span
                                                    className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${sub.Tier === 'pro'
                                        ...
                                        <td className="p-4 text-right">
                                            {isMasterSub ? (
                                                <span
                                                    className="text-xs text-indigo-500 font-bold px-2">Protected</span>
                                            ) : isEditing ? (
                ...
                {activeTab === "metrics" && metrics && (
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div
                                className="bg-[var(--card)] p-6 rounded-2xl border border-[color:var(--border)] flex items-center gap-4">
                                <div className="p-4 bg-indigo-500/10 text-indigo-500 rounded-xl"><Users size={24}/>
                                </div>
                                <div>
                                    <p className="text-sm text-[color:var(--muted-foreground)] font-medium">Total
                                        Registered Users</p>
                            ...
                            <div
                                className="bg-[var(--card)] p-6 rounded-2xl border border-[color:var(--border)] flex items-center gap-4">
                                <div className="p-4 bg-emerald-500/10 text-emerald-500 rounded-xl"><CreditCard
                                    size={24}/></div>
                                <div>
                                    <p className="text-sm text-[color:var(--muted-foreground)] font-medium">Live Gross
                                        Revenue</p>
                            ...
                            <div
                                className="bg-[var(--card)] p-6 rounded-2xl border border-[color:var(--border)] flex items-center gap-4">
                                <div className="p-4 bg-amber-500/10 text-amber-500 rounded-xl"><BarChart3 size={24}/>
                                </div>
                                <div>
                                    <p className="text-sm text-[color:var(--muted-foreground)] font-medium">Total
                                        Actions Processed</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div
                                className="bg-[var(--card)] p-6 rounded-2xl border border-[color:var(--border)] lg:col-span-2">
                                ...
                                <div className="flex flex-wrap justify-center gap-3 text-xs mt-2">
                                    {(metrics.sub_distribution || []).map((d: any) => (
                                        <div key={d.status}
                                            ...
                                            <span
                                                className={`w-3 h-3 rounded-full ${d.status === 'active' ? 'bg-emerald-500' : d.status === 'canceled' ? 'bg-amber-500' : d.status === 'past_due' ? 'bg-red-500' : 'bg-gray-500'}`}></span>
                                            {d.status}: {d.count}
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="h-72">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={metrics.tool_usage || []}>
                                        ...
                                        <XAxis dataKey="tool_name" stroke="#888888" fontSize={12} tickLine={false}
                                               axisLine={false}/>
                                        <YAxis stroke="#888888" fontSize={12} tickLine={false} axisLine={false}/>
                                        <Tooltip contentStyle={{
                                            backgroundColor: 'var(--card)',
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between border-b border-[color:var(--border)] pb-4">
                            ...
                            <button onClick={() => setSelectedUserDetail(null)}
                                    className="p-2 bg-[color:var(--background)] border border-[color:var(--border)] rounded-xl hover:text-indigo-500 transition">
                                <X size={18}/>
                            </button>
                        </div>
                        ...
                        {/* User Overview */}
                        <div
                            className="bg-[color:var(--background)]/60 border border-[color:var(--border)] rounded-2xl p-5 space-y-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <span
                                        className="font-mono text-xs block mt-1">{new Date(selectedUserDetail.user.CreatedAt).toLocaleString()}</span>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-[color:var(--border)] pt-4 flex flex-wrap gap-2">
                                ...
                                >
                                    {selectedUserDetail.user.Status === 'active' ? <><Ban size={14}/> Ban
                                        Account</> : <><CheckCircle size={14}/> Lift Account Ban</>}
                                </button>
                        ...
                        <div className="border border-[color:var(--border)] bg-[var(--card)] rounded-2xl p-5 space-y-4">
                            ...
                            {selectedUserDetail.user.Email === MASTER_EMAIL ? (
                                <p className="text-xs text-emerald-500 font-bold bg-emerald-500/5 border border-emerald-500/10 p-3 rounded-xl flex items-center gap-2">
                                    <ShieldCheck size={14}/> Master Administrator profile possesses absolute lifelong
                                    tier clearance exemptions.
                                </p>
                            ) : selectedUserDetail.subscription ? (
                                <div className="space-y-4 text-xs">
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                        <div>
                                            <label className="text-[color:var(--muted-foreground)] block mb-1">Quota
                                                Tier</label>
                                            <select
                                                value={editTier}
                                                onChange={(e) => setEditTier(e.target.value)}
                                        ...
                                        <div>
                                            <label className="text-[color:var(--muted-foreground)] block mb-1">Paddle
                                                Status</label>
                                            <select
                                                value={editStatus}
                                                onChange={(e) => setEditStatus(e.target.value)}
                                        ...
                                        <div>
                                            <label className="text-[color:var(--muted-foreground)] block mb-1">Add Time
                                                Window</label>
                                            <input
                                                type="number"
                                                placeholder="Days"
                                ...
                                <p className="text-xs text-[color:var(--muted-foreground)]">No registration tier row
                                    found.</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-3">
                            <h3 className="font-bold text-sm flex items-center gap-2">
                                <History size={16} className="text-indigo-500"/> Transaction Ledger History
                                ({selectedUserDetail.transactions?.length || 0})
                            </h3>
                            <div
                                className="max-h-40 overflow-y-auto border border-[color:var(--border)] rounded-xl text-xs bg-[var(--background)]">
                                {selectedUserDetail.transactions?.length === 0 ? (
                                    <p className="p-4 text-[color:var(--muted-foreground)] text-center">No transactions
                                        on record.</p>
                                ) : (
                                    <table className="w-full text-left">
                                        <thead
                                            className="bg-[color:var(--background)] sticky top-0 border-b border-[color:var(--border)]">
                                        <tr>
                                        ...
                                        {selectedUserDetail.transactions.map((tx: any) => (
                                            <tr key={tx.ID}
                                                className="border-b border-[color:var(--border)] last:border-0 hover:bg-[color:var(--card)]">
                        ...
                        <div className="space-y-3 flex-1 flex flex-col pb-4">
                            <h3 className="font-bold text-sm flex items-center gap-2">
                                <BarChart3 size={16} className="text-indigo-500"/> Recent Document Activity Logs
                                ({selectedUserDetail.usage_logs?.length || 0})
                            </h3>
                            <div
                                className="flex-1 max-h-56 overflow-y-auto border border-[color:var(--border)] rounded-xl text-xs bg-[var(--background)]">
                                {selectedUserDetail.usage_logs?.length === 0 ? (
                                    <p className="p-4 text-[color:var(--muted-foreground)] text-center">No operational
                                        logs captured within current parameters window.</p>
                                ) : (
                                    <table className="w-full text-left">
                                        <thead
                                            className="bg-[color:var(--background)] sticky top-0 border-b border-[color:var(--border)]">
