...
export default function SharedToolLayout({ children }: { children: ReactNode }) {
        ...
        fetchJson("/site-content/tools")
            .then((data: unknown) => {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    if (found) {
                        setToolConfig({
                            name: found.Name || found.name || found.Title || found.title || activeFallback?.title || "PDF Tool",
                            description: found.Description || found.description || activeFallback?.description || "Process files securely.",
                            icon: found.Icon || found.icon || activeFallback?.icon || null,
                            multiple: found.Multiple !== undefined ? found.Multiple : (found.multiple !== undefined ? found.multiple : (activeFallback?.multiple || false)),
            ...
            .catch((err) => {
                console.error("Error fetching remote tool config context layout, falling back:", err);
                const activeFallback = NAV_TOOLS.find((t) => t.href === `/${toolId}`) as ToolItem | undefined;
                if (activeFallback) {
                    setToolConfig({
                        name: activeFallback.title,
    ...
    return (
        <ToolContext.Provider value={{ toolId, toolConfig, file, setFile, downloadData, setDownloadData, isLoadingConfig }}>
            <ToolSEO href={`/${toolId}`} />
            {children}
