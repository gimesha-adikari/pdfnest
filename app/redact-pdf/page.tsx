    ...
    return (
        <PdfToolLayout>
            ...
            <div className="mt-12 rounded-3xl border border-[color:var(--border)] bg-[var(--card)] p-8 shadow-lg">
                <PdfUploader onFilesAccepted={onDrop} title="Upload Document" description="Select file to redact" multiple={false} accept=".pdf" />
                ...
                {file && pdfDoc && (
                    <div className="mt-8 space-y-6">
                        ...
                        <div className="grid gap-6 lg:grid-cols-3">
                            {/* Side Controls Panel */}
                            <div className="space-y-4 rounded-2xl border border-[color:var(--border)] p-5 bg-[color:var(--background)]/50 h-fit">
                                ...
                                <div className="grid grid-cols-2 gap-1 rounded-xl bg-zinc-100 dark:bg-zinc-800 p-1">
                                    <button
                                        ...
                                        className={`flex items-center justify-center gap-2 py-2 text-xs font-medium rounded-lg transition ${mode === "text" ? "bg-white shadow dark:bg-zinc-700" : "text-zinc-500"}`}
                                    >
                                        <Type size={14} /> Text Search
                                    </button>
                                    <button
                                        onClick={() => setMode("draw")}
                                        className={`flex items-center justify-center gap-2 py-2 text-xs font-medium rounded-lg transition ${mode === "draw" ? "bg-white shadow dark:bg-zinc-700" : "text-zinc-500"}`}
                                    >
                                ...
                                {mode === "text" ? (
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            className="mt-1.5 w-full h-24 text-sm rounded-xl border border-[color:var(--border)] bg-[color:var(--card)] p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                        />
                                    </div>
                                ) : (
                                        ...
                                        {drawnBoxes.length > 0 && (
                                            <div className="border-t pt-2 max-h-40 overflow-y-auto space-y-1">
                                                ...
                                                {drawnBoxes.map((box, idx) => (
                                                    <div key={box.id} className="flex items-center justify-between bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 rounded-lg text-xs">
                            ...
                            <div className="lg:col-span-2 flex flex-col gap-6 p-6 border rounded-2xl bg-zinc-900/5 max-h-[700px] overflow-y-auto overflow-x-hidden" ref={containerRef}>
                                {Array.from({ length: totalPages }, (_, i) => (
                                    <div key={i} className="relative shadow-md flex flex-col items-center select-none">
                                        <canvas
